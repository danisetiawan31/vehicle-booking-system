...
class BookingService
{
    ...
    public function createBooking(array $data, int $adminId, int $approverLevel1Id, int $approverLevel2Id): array
    {
        ...
        try {
            // 1. Lock vehicle row first (Order: vehicle -> driver)
            $db->query("SELECT id FROM vehicles WHERE id = ? FOR UPDATE", [(int)$data['vehicle_id']]);

            // 2. Lock driver row next
            $db->query("SELECT id FROM drivers WHERE id = ? FOR UPDATE", [(int)$data['driver_id']]);

            // 3. Perform overlap checks within transaction
